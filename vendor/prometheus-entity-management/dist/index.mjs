...
function useEntityView(opts) {
  ...
  const optsRef = useRef(opts);
  optsRef.current = opts;
  const [liveView, setLiveView] = useState(opts.view);
  ...
  const setView = useCallback((partial) => setLiveView((prev) => ({ ...prev, ...partial })), []);
  const setFilter = useCallback((filter) => setLiveView((prev) => ({ ...prev, filter: filter ?? void 0 })), []);
  const setSort = useCallback((sort) => setLiveView((prev) => ({ ...prev, sort: sort ?? void 0 })), []);
  const setSearch = useCallback((query) => setLiveView((prev) => ({ ...prev, search: prev.search ? { ...prev.search, query } : { query, fields: [] } })), []);
...
function useEntityCRUD(opts) {
  ...
  const relations = useMemo(() => detail ? readRelations(type, detail) : {}, [type, detail]);
...
function useLocalFirst(adapter) {
  ...
  const query = useCallback(async (sql, params) => (await adapter.query(sql, params)).rows, [adapter]);
  const execute = useCallback((sql, params) => adapter.execute(sql, params), [adapter]);
...
function useGQLList(opts) {
  ...
  optsRef.current = opts;
  const key = useMemo(() => serializeKey(queryKey), [queryKey]);
  const listState = useStore(useGraphStore, useCallback((s) => s.lists[key] ?? EMPTY_LIST_STATE, [key]));
...
function EntityTable({ viewResult, columns, getRowId = (r) => String(r.id), selectedId, onRowClick, onCellEdit, onBulkAction, paginationMode = "loadMore", pageSize = 50, searchPlaceholder = "Search\u2026", searchFields, toolbarChildren, showToolbar = true, emptyState, className }) {
  ...
  const handleSort = useCallback((updater) => {
  ...
  return /* @__PURE__ */ jsxs("div", { className: cn("flex flex-col h-full", className), children: [
    showToolbar && /* @__PURE__ */ jsxs("div", { className: "flex items-center gap-2 px-3 py-2.5 border-b shrink-0", children: [
      /* @__PURE__ */ jsxs("div", { className: "relative flex-1 max-w-xs", children: [
        ...
        search && /* @__PURE__ */ jsx("button", { onClick: () => handleSearch(""), className: "absolute right-2.5 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground", children: /* @__PURE__ */ jsx(X, { className: "w-3.5 h-3.5" }) })
      ] }),
      isShowingLocalPending && /* @__PURE__ */ jsxs("span", { className: "text-[10px] text-muted-foreground flex items-center gap-1", children: [
      ...
      /* @__PURE__ */ jsxs("div", { className: "ml-auto flex items-center gap-1.5", children: [
        selectedRows.length > 0 && onBulkAction && /* @__PURE__ */ jsxs(Fragment, { children: [
        ...
        /* @__PURE__ */ jsx("span", { className: "text-xs text-muted-foreground", children: viewTotal != null ? `${viewTotal}` : "" }),
        /* @__PURE__ */ jsx("button", { onClick: refetch, disabled: isRemoteFetching, className: "p-1 rounded hover:bg-muted text-muted-foreground hover:text-foreground transition-colors", children: /* @__PURE__ */ jsx(RefreshCw, { className: cn("w-3.5 h-3.5", isRemoteFetching && "animate-spin") }) }),
        toolbarChildren
      ] })
    ] }),
    /* @__PURE__ */ jsx("div", { className: "flex-1 overflow-auto", children: /* @__PURE__ */ jsxs("table", { className: "w-full border-collapse text-sm", children: [
      /* @__PURE__ */ jsx("thead", { className: "sticky top-0 z-10", children: table.getHeaderGroups().map((hg) => /* @__PURE__ */ jsx("tr", { className: "bg-muted/50 border-b", children: hg.headers.map((h) => /* @__PURE__ */ jsx("th", { style: { width: h.getSize() }, className: "px-3 py-2 text-left font-normal text-muted-foreground", children: h.isPlaceholder ? null : flexRender(h.column.columnDef.header, h.getContext()) }, h.id)) }, hg.id)) }),
      /* @__PURE__ */ jsx("tbody", { children: isLoading ? Array.from({ length: 6 }).map((_, i) => /* @__PURE__ */ jsx("tr", { className: "border-b", children: columns.map((_2, j) => /* @__PURE__ */ jsx("td", { className: "px-3 py-2.5", children: /* @__PURE__ */ jsx("div", { className: "h-3 rounded bg-muted animate-pulse", style: { width: `${50 + (i * 7 + j * 13) % 40}%` } }) }, j)) }, i)) : table.getRowModel().rows.length === 0 ? /* @__PURE__ */ jsx("tr", { children: /* @__PURE__ */ jsx("td", { colSpan: columns.length, className: "px-4 py-16 text-center text-sm text-muted-foreground", children: emptyState ?? "No results" }) }) : table.getRowModel().rows.map((row) => {
            ...
            children: row.getVisibleCells().map((cell) => {
              ...
              return /* @__PURE__ */ jsx(
                "td",
                {
                  ...
                  children: isEditingThis ? /* @__PURE__ */ jsx(InlineCellEditor, { initialValue: editingCell.value, onCommit: (v) => {
                    onCellEdit(row.original, cell.column.id, v);
                    setEditingCell(null);
                  }, onCancel: () => setEditingCell(null) }) : flexRender(cell.column.columnDef.cell, cell.getContext())
                },
                cell.id
              );
            })
    ...
    /* @__PURE__ */ jsxs("div", { className: "flex items-center justify-between px-3 py-2 border-t shrink-0 text-xs text-muted-foreground", children: [
      /* @__PURE__ */ jsx("span", { children: isFetching && !isLoading ? "Updating\u2026" : `${items.length} loaded` }),
      paginationMode === "loadMore" && hasNextPage && /* @__PURE__ */ jsx("button", { onClick: fetchNextPage, disabled: isFetchingMore, className: "flex items-center gap-1.5 px-2.5 py-1 rounded border text-xs hover:bg-muted transition-colors disabled:opacity-50", children: isFetchingMore ? /* @__PURE__ */ jsxs(Fragment, { children: [
        /* @__PURE__ */ jsx(Loader2, { className: "w-3 h-3 animate-spin" }),
        "Loading\u2026"
      ] }) : "Load more" }),
      paginationMode === "pages" && totalPages > 1 && /* @__PURE__ */ jsxs("div", { className: "flex items-center gap-1", children: [
        /* @__PURE__ */ jsx("button", { onClick: () => setPage((p) => Math.max(1, p - 1)), disabled: page === 1, className: "p-1 rounded hover:bg-muted disabled:opacity-50", children: /* @__PURE__ */ jsx(ChevronLeft, { className: "w-3.5 h-3.5" }) }),
        ...
        /* @__PURE__ */ jsx("button", { onClick: () => setPage((p) => Math.min(totalPages, p + 1)), disabled: page >= totalPages, className: "p-1 rounded hover:bg-muted disabled:opacity-50", children: /* @__PURE__ */ jsx(ChevronRight, { className: "w-3.5 h-3.5" }) })
      ] })
    ] })
  ] });
}
function Sheet({ open, onClose, title, subtitle, children, footer, width = "w-[480px]" }) {
  ...
  return /* @__PURE__ */ jsxs(Fragment, { children: [
    /* @__PURE__ */ jsx("div", { onClick: onClose, className: cn("fixed inset-0 z-40 bg-black/40 backdrop-blur-sm transition-opacity", open ? "opacity-100" : "opacity-0 pointer-events-none") }),
    /* @__PURE__ */ jsxs("div", { className: cn("fixed top-0 right-0 h-full z-50 flex flex-col border-l bg-background shadow-2xl transition-transform duration-300", width, open ? "translate-x-0" : "translate-x-full"), children: [
      /* @__PURE__ */ jsxs("div", { className: "flex items-start justify-between gap-3 px-5 pt-5 pb-4 border-b shrink-0", children: [
        ...
        /* @__PURE__ */ jsx("button", { onClick: onClose, className: "p-1 rounded hover:bg-muted text-muted-foreground hover:text-foreground transition-colors mt-0.5", children: /* @__PURE__ */ jsx(X, { className: "w-4 h-4" }) })
      ] }),
      /* @__PURE__ */ jsx("div", { className: "flex-1 overflow-y-auto px-5 py-4", children }),
      footer && /* @__PURE__ */ jsx("div", { className: "shrink-0 px-5 py-4 border-t flex items-center gap-2", children: footer })
    ] })
  ] });
}
function FieldControl({ descriptor, value, onChange, entity, readonly }) {
  if (descriptor.editControl && !readonly) return /* @__PURE__ */ jsx(Fragment, { children: descriptor.editControl(value, onChange, entity) });
  if (readonly && descriptor.render) return /* @__PURE__ */ jsx(Fragment, { children: descriptor.render(value, entity) });
  if (readonly) return /* @__PURE__ */ jsx("p", { className: "text-sm py-1", children: value != null && value !== "" ? String(value) : "\u2014" });
  ...
  switch (descriptor.type) {
    ...
    case "email":
    case "url":
      return /* @__PURE__ */ jsx("input", { type: descriptor.type, value: String(value ?? ""), onChange: (e) => onChange(e.target.value), placeholder: descriptor.placeholder, className: base });
    case "number":
    ...
    case "boolean":
      return /* @__PURE__ */ jsx(
        "button",
        {
          ...
          children: /* @__PURE__ */ jsx("span", { className: cn("inline-block h-3.5 w-3.5 rounded-full bg-background shadow transition-transform", value ? "translate-x-4" : "translate-x-0.5") })
        }
      );
    case "enum":
      return /* @__PURE__ */ jsxs("select", { value: String(value ?? ""), onChange: (e) => onChange(e.target.value), className: cn(base, "appearance-none cursor-pointer"), children: [
  ...
  return /* @__PURE__ */ jsxs(
    Sheet,
    {
      ...
      footer: /* @__PURE__ */ jsxs(Fragment, { children: [
        showEditButton && /* @__PURE__ */ jsxs("button", { onClick: () => crud.startEdit(), className: "flex-1 h-8 rounded-md border text-sm hover:bg-muted transition-colors flex items-center justify-center gap-1.5", children: [
          /* @__PURE__ */ jsx(Pencil, { className: "w-3.5 h-3.5" }),
          "Edit"
        ] }),
        showDeleteButton && /* @__PURE__ */ jsx("button", { onClick: () => setConfirmDelete(true), className: "h-8 w-8 flex items-center justify-center rounded-md border text-destructive hover:bg-destructive/10 transition-colors", children: /* @__PURE__ */ jsx(Trash2, { className: "w-3.5 h-3.5" }) })
      ] }),
      children: [
        crud.detailIsLoading && /* @__PURE__ */ jsx("div", { className: "flex items-center justify-center h-32", children: /* @__PURE__ */ jsx(Loader2, { className: "w-4 h-4 animate-spin text-muted-foreground" }) }),
        ...
        confirmDelete && /* @__PURE__ */ jsxs("div", { className: "fixed inset-0 z-50 flex items-center justify-center p-4", children: [
          /* @__PURE__ */ jsx("div", { onClick: () => setConfirmDelete(false), className: "absolute inset-0 bg-black/50 backdrop-blur-sm" }),
          /* @__PURE__ */ jsxs("div", { className: "relative bg-background border rounded-xl shadow-2xl w-full max-w-sm p-5 animate-in fade-in", children: [
            /* @__PURE__ */ jsxs("h3", { className: "text-sm font-semibold mb-1", children: [
              "Delete ",
              resolvedTitle,
              "?"
            ] }),
            /* @__PURE__ */ jsx("p", { className: "text-xs text-muted-foreground mb-4", children: deleteConfirmMessage }),
            /* @__PURE__ */ jsxs("div", { className: "flex gap-2 justify-end", children: [
              /* @__PURE__ */ jsx("button", { onClick: () => setConfirmDelete(false), className: "h-7 px-3 rounded border text-sm hover:bg-muted transition-colors", children: "Cancel" }),
              /* @__PURE__ */ jsxs(
                "button",
                {
                  onClick: async () => {
                  ...
                  children: [
                    crud.isDeleting && /* @__PURE__ */ jsx(Loader2, { className: "w-3 h-3 animate-spin" }),
                    "Delete"
                  ]
                }
              )
...
function EntityFormSheet({ crud, fields, createTitle = "Create", editTitle = "Edit" }) {
  ...
  return /* @__PURE__ */ jsx(
    Sheet,
    {
      ...
      footer: /* @__PURE__ */ jsxs(Fragment, { children: [
        /* @__PURE__ */ jsx("button", { onClick: handleClose, disabled: isPending, className: "flex-1 h-8 rounded-md border text-sm hover:bg-muted transition-colors", children: "Cancel" }),
        /* @__PURE__ */ jsxs("button", { onClick: handleSave, disabled: isPending || isEdit && !crud.dirty.isDirty, className: "flex-1 h-8 rounded-md bg-primary text-primary-foreground text-sm hover:bg-primary/90 transition-colors disabled:opacity-50 flex items-center justify-center gap-1.5", children: [
          isPending && /* @__PURE__ */ jsx(Loader2, { className: "w-3.5 h-3.5 animate-spin" }),
...
function SortHeader({ column, label }) {
  ...
  return /* @__PURE__ */ jsxs(
    "button",
    {
      ...
      children: [
        label,
        /* @__PURE__ */ jsx("span", { className: "opacity-50 group-hover:opacity-100 text-[10px]", children: sorted === "asc" ? "\u2191" : sorted === "desc" ? "\u2193" : "\u2195" })
      ]
    }
  );
}
...
function textColumn(opts) {
  ...
  return {
    ...
    accessorKey: field,
    size,
    header: ({ column }) => /* @__PURE__ */ jsx(SortHeader, { column, label: header }),
    cell: ({ getValue, row }) => {
      const v = getValue();
      return cell ? cell(v, row.original) : /* @__PURE__ */ jsx("span", { className: "truncate block", title: v, children: v });
    },
...
function numberColumn(opts) {
  ...
  return {
    ...
    cell: ({ getValue }) => {
      const v = getValue();
      return /* @__PURE__ */ jsx("span", { className: "tabular-nums text-right block", children: v != null ? format(v) : "\u2014" });
    },
...
function dateColumn(opts) {
  ...
  return {
    ...
    cell: ({ getValue }) => {
      const v = getValue();
      return v ? /* @__PURE__ */ jsx("span", { className: "text-sm", children: new Date(v).toLocaleDateString(void 0, format) }) : /* @__PURE__ */ jsx("span", { className: "text-muted-foreground", children: "\u2014" });
    },
...
function booleanColumn(opts) {
  ...
  return {
    ...
    cell: ({ getValue }) => {
      const v = getValue();
      return /* @__PURE__ */ jsx("span", { className: v ? "text-green-600" : "text-muted-foreground", children: v ? trueLabel : falseLabel });
    },
...
function enumColumn(opts) {
  ...
  return {
    ...
    cell: ({ getValue }) => {
      const v = getValue();
      const opt = map.get(v);
      return opt ? /* @__PURE__ */ jsx("span", { className: `inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium border ${opt.className ?? "bg-muted text-muted-foreground border-border"}`, children: opt.label }) : /* @__PURE__ */ jsx("span", { className: "text-xs text-muted-foreground", children: v });
    },
...
function actionsColumn(actions) {
  return {
    ...
    enableHiding: false,
    header: () => null,
    cell: ({ row }) => /* @__PURE__ */ jsx("div", { className: "flex items-center gap-0.5 opacity-0 group-hover/row:opacity-100 transition-opacity", children: actions.filter((a) => !a.hidden?.(row.original)).map((action) => /* @__PURE__ */ jsx(
      "button",
      {
        type: "button",
        onClick: (e) => {
        ...
        children: action.icon ? /* @__PURE__ */ jsx(action.icon, { className: "w-3.5 h-3.5" }) : action.label
      },
      action.label
    )) })
  };
...
function useTable(options) {
  ...
  const [columnFilters, _setColumnFilters] = useState(
  ...
  const [columnPinning, _setColumnPinning] = useState(
  ...
  const [columnSizingInfo, _setColumnSizingInfo] = useState(
  ...
  const [grouping, _setGrouping] = useState(
  ...
  const setColumnSizingInfo = useCallback((updater) => {
  ...
  const coreRowModelRef = useRef(null);
  const emptyCoreRowModel = useMemo(
...
var TableStorageContext = createContext({
...
function TableStorageProvider({
  ...
  return /* @__PURE__ */ jsx(TableStorageContext.Provider, { value, children });
}
  ...
  return /* @__PURE__ */ jsxs(
    "button",
    {
      ...
      children: [
        ...
        /* @__PURE__ */ jsx("span", { className: "ml-1 flex flex-col text-xs leading-none", children: sorted === "asc" ? /* @__PURE__ */ jsx(SortAscIcon, { className: "h-3.5 w-3.5" }) : sorted === "desc" ? /* @__PURE__ */ jsx(SortDescIcon, { className: "h-3.5 w-3.5" }) : /* @__PURE__ */ jsx(SortNoneIcon, { className: "h-3.5 w-3.5 text-muted-foreground/50" }) }),
        column.getSortIndex() >= 0 && /* @__PURE__ */ jsx("span", { className: "ml-0.5 text-[10px] text-muted-foreground tabular-nums", children: column.getSortIndex() + 1 })
      ]
    }
  );
}
function SortAscIcon({ className }) {
  return /* @__PURE__ */ jsxs("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 16 16", fill: "currentColor", className, "aria-hidden": true, children: [
    /* @__PURE__ */ jsx("title", { children: "Sorted ascending" }),
    /* @__PURE__ */ jsx("path", { fillRule: "evenodd", d: "M8 3.5a.5.5 0 01.354.146l3.5 3.5a.5.5 0 11-.708.708L8 4.707 4.854 7.854a.5.5 0 11-.708-.708l3.5-3.5A.5.5 0 018 3.5z" })
  ] });
}
function SortDescIcon({ className }) {
  return /* @__PURE__ */ jsxs("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 16 16", fill: "currentColor", className, "aria-hidden": true, children: [
    /* @__PURE__ */ jsx("title", { children: "Sorted descending" }),
    /* @__PURE__ */ jsx("path", { fillRule: "evenodd", d: "M8 12.5a.5.5 0 01-.354-.146l-3.5-3.5a.5.5 0 11.708-.708L8 11.293l3.146-3.147a.5.5 0 11.708.708l-3.5 3.5A.5.5 0 018 12.5z" })
  ] });
}
function SortNoneIcon({ className }) {
  return /* @__PURE__ */ jsxs("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 16 16", fill: "currentColor", className, "aria-hidden": true, children: [
    /* @__PURE__ */ jsx("title", { children: "Not sorted" }),
    /* @__PURE__ */ jsx("path", { d: "M8 3.5a.5.5 0 01.354.146l2.5 2.5a.5.5 0 11-.708.708L8 4.707 5.854 6.854a.5.5 0 11-.708-.708l2.5-2.5A.5.5 0 018 3.5z" }),
    /* @__PURE__ */ jsx("path", { d: "M8 12.5a.5.5 0 01-.354-.146l-2.5-2.5a.5.5 0 11.708-.708L8 11.293l2.146-2.147a.5.5 0 11.708.708l-2.5 2.5A.5.5 0 018 12.5z" })
  ] });
}
function InlineCellEditor2({
  ...
  if (filterType === "boolean") {
    return /* @__PURE__ */ jsx("div", { className: cn("flex items-center", className), children: /* @__PURE__ */ jsx(
      "input",
      {
        id: inputId,
        type: "checkbox",
      ...
      children: [
        editableFields.map((col) => {
          ...
          return /* @__PURE__ */ jsxs("div", { className: "space-y-1", children: [
            /* @__PURE__ */ jsx("label", { htmlFor: fieldId, className: "text-xs font-medium text-muted-foreground", children: label }),
            /* @__PURE__ */ jsx(
              InlineCellEditor2,
              {
                value: currentValue,
...
function ActionDropdown({
  ...
  return /* @__PURE__ */ jsxs("div", { className: cn("relative inline-block", className), ref: menuRef, children: [
    /* @__PURE__ */ jsxs(
      "button",
      {
        type: "button",
        ...
        children: [
          /* @__PURE__ */ jsx(MoreIcon, { className: "h-4 w-4" }),
          /* @__PURE__ */ jsx("span", { className: "sr-only", children: "Actions" })
        ]
      }
    ),
    isOpen && /* @__PURE__ */ jsx("div", { className: "absolute right-0 z-50 mt-1 min-w-[160px] rounded-md border bg-popover py-1 shadow-md", children: visibleActions.map((action) => /* @__PURE__ */ jsxs(
      "button",
      {
        type: "button",
...
function ActionButtonRow({
  ...
  return /* @__PURE__ */ jsxs("div", { className: cn("flex items-center gap-1", className), children: [
    inline.map((action) => /* @__PURE__ */ jsx(
      "button",
      {
        type: "button",
        onClick: () => executeAction(action),
        ...
        children: action.icon && /* @__PURE__ */ jsx(action.icon, { className: "h-3.5 w-3.5" })
      },
      action.id
    )),
    overflow.length > 0 && /* @__PURE__ */ jsx(ActionDropdown, { item, actions: overflow }),
    confirmAction && /* @__PURE__ */ jsx(
      ConfirmDialog,
      {
        message: typeof confirmAction.confirm === "function" ? confirmAction.confirm(item) : confirmAction.confirm,
...
function ConfirmDialog({
  ...
  return /* @__PURE__ */ jsx("div", { className: "fixed inset-0 z-50 flex items-center justify-center bg-black/50", children: /* @__PURE__ */ jsxs("div", { className: "w-full max-w-sm rounded-lg border bg-background p-6 shadow-lg", children: [
...
function MoreIcon({ className }) {
  return /* @__PURE__ */ jsxs(
    "svg",
    {
      ...
      "aria-hidden": true,
      children: [
        /* @__PURE__ */ jsx("title", { children: "More actions" }),
...
function EyeIcon({ className }) {
  return /* @__PURE__ */ jsxs(
    "svg",
    {
      ...
      "aria-hidden": true,
      children: [
        /* @__PURE__ */ jsx("title", { children: "View" }),
...
function PencilIcon({ className }) {
  return /* @__PURE__ */ jsxs(
    "svg",
    {
      ...
      "aria-hidden": true,
      children: [
        /* @__PURE__ */ jsx("title", { children: "Edit" }),
...
function TrashIcon({ className }) {
  return /* @__PURE__ */ jsxs(
    "svg",
    {
      ...
      "aria-hidden": true,
      children: [
        /* @__PURE__ */ jsx("title", { children: "Delete" }),
...
function DataTable({
  ...
  return /* @__PURE__ */ jsxs(Table, { className, children: [
    /* @__PURE__ */ jsx(TableHeader, { children: headerGroups.map((headerGroup) => /* @__PURE__ */ jsxs(TableRow, { children: [
      enableMultiSelect && selectionStore && /* @__PURE__ */ jsx(TableHead, { className: "w-[40px]", children: /* @__PURE__ */ jsx(
        SelectAllCheckbox,
        {
          table,
      ...
      headerGroup.headers.map((header) => /* @__PURE__ */ jsx(
        TableHead,
        {
          ...
          children: header.isPlaceholder ? null : /* @__PURE__ */ jsx(
      ...
      actions && actions.length > 0 && /* @__PURE__ */ jsx(TableHead, { className: "w-[50px] text-right", children: /* @__PURE__ */ jsx("span", { className: "sr-only", children: "Actions" }) })
    ] }, headerGroup.id)) }),
    /* @__PURE__ */ jsx(TableBody, { children: rowModel.rows.length === 0 ? /* @__PURE__ */ jsx(TableRow, { children: /* @__PURE__ */ jsx(
      TableCell,
      {
    ...
    ) }) : rowModel.rows.map((row) => /* @__PURE__ */ jsx(
      DataTableRow,
      {
        row,
        table,
...
function DataTableRow({
  ...
  return /* @__PURE__ */ jsxs(TableRow, { "data-state": isSelected ? "selected" : void 0, children: [
    enableMultiSelect && selectionStore && /* @__PURE__ */ jsx(TableCell, { className: "w-[40px]", children: /* @__PURE__ */ jsx(RowCheckbox, { store: selectionStore, rowId }) }),
    row.getVisibleCells().map((cell) => {
      ...
      return /* @__PURE__ */ jsx(
        TableCell,
        {
          ...
          ) : /* @__PURE__ */ jsx(CellRenderer, { cell })
        },
        cell.id
      );
    }),
    actions && actions.length > 0 && /* @__PURE__ */ jsx(TableCell, { className: "w-[50px] text-right", children: /* @__PURE__ */ jsx(ActionDropdown, { item: row.original, actions }) })
  ] });
}
function CellRenderer({ cell }) {
  const cellDef = cell.column.columnDef.cell;
  if (typeof cellDef === "function") {
    return /* @__PURE__ */ jsx(Fragment, { children: cellDef(cell.getContext()) });
  }
  ...
  if (renderCard) {
    return /* @__PURE__ */ jsxs(
      "div",
      {
        ...
        children: [
          enableMultiSelect && isMultiSelectMode && /* @__PURE__ */ jsx("div", { className: "absolute left-2 top-2 z-10", children: /* @__PURE__ */ jsx(
            "input",
            {
              type: "checkbox",
              checked: isSelected,
  ...
  return /* @__PURE__ */ jsxs(
    "div",
    {
      ...
      children: [
        ...
        desc?.image && !!item[desc.image] && /* @__PURE__ */ jsx("div", { className: "aspect-video w-full overflow-hidden rounded-t-lg bg-muted", children: /* @__PURE__ */ jsx(
          "img",
          {
        ...
        /* @__PURE__ */ jsxs("div", { className: "flex flex-1 flex-col gap-1 p-4", children: [
          /* @__PURE__ */ jsxs("div", { className: "flex items-start gap-2", children: [
            desc?.avatar && !!item[desc.avatar] && /* @__PURE__ */ jsx(
              "img",
              {
            ...
            /* @__PURE__ */ jsxs("div", { className: "flex-1 min-w-0", children: [
              desc?.title && /* @__PURE__ */ jsx("h3", { className: "truncate font-medium text-sm", children: String(item[desc.title] ?? "") }),
              desc?.subtitle && /* @__PURE__ */ jsx("p", { className: "truncate text-xs text-muted-foreground", children: String(item[desc.subtitle] ?? "") })
            ] })
          ] }),
          desc?.description && !!item[desc.description] && /* @__PURE__ */ jsx("p", { className: "mt-1 text-xs text-muted-foreground line-clamp-2", children: String(item[desc.description]) }),
          desc?.badges && desc.badges.length > 0 && /* @__PURE__ */ jsx("div", { className: "mt-2 flex flex-wrap gap-1", children: desc.badges.map((badge) => {
          ...
          desc?.metadata && desc.metadata.length > 0 && /* @__PURE__ */ jsx("div", { className: "mt-2 space-y-0.5", children: desc.metadata.map((meta) => {
            const val = item[meta.field];
            return /* @__PURE__ */ jsxs("div", { className: "flex items-center gap-1 text-[11px] text-muted-foreground", children: [
              /* @__PURE__ */ jsxs("span", { className: "font-medium", children: [
                meta.label,
                ":"
              ] }),
              /* @__PURE__ */ jsx("span", { children: meta.format ? meta.format(val) : String(val ?? "") })
            ] }, meta.field);
          }) })
        ] }),
        isEditing && /* @__PURE__ */ jsx("div", { className: "border-t p-4", children: /* @__PURE__ */ jsx(
          InlineItemEditor,
          {
            item,
            columns,
        ...
        !isEditing && actions.length > 0 && /* @__PURE__ */ jsx("div", { className: "flex items-center justify-end border-t px-3 py-2", children: /* @__PURE__ */ jsx(ActionButtonRow, { item, actions }) })
      ]
    }
  );
}
...
function ListView({
  ...
  const [editingId, setEditingId] = useState(null);
  return /* @__PURE__ */ jsx("div", { className: cn("divide-y rounded-md border", className), children: rows.map((row) => {
    const id = getRowId?.(row.original) ?? row.id;
    const isEditing = editingId === id;
    return /* @__PURE__ */ jsx(
      ListItem,
...
function ListItem({
  ...
  return /* @__PURE__ */ jsxs(
    "div",
    {
      ...
      children: [
        /* @__PURE__ */ jsxs("div", { className: "flex items-center gap-3 px-4 py-3 hover:bg-muted/30", children: [
          enableMultiSelect && isMultiSelectMode && /* @__PURE__ */ jsx(
            "input",
            {
              type: "checkbox",
              checked: isSelected,
          ...
          /* @__PURE__ */ jsxs("div", { className: "flex min-w-0 flex-1 items-center gap-3", children: [
            /* @__PURE__ */ jsx(
              "div",
              {
                ...
                ...rowEditProps,
                children: renderItem ? renderItem(item, context) : /* @__PURE__ */ jsx(DefaultListItemContent, { item, descriptor: desc })
              }
            ),
            !isEditing && actions.length > 0 && /* @__PURE__ */ jsx("div", { className: "flex-shrink-0", children: /* @__PURE__ */ jsx(ActionButtonRow, { item, actions, maxVisible: 2 }) })
          ] })
        ] }),
        isEditing && /* @__PURE__ */ jsx("div", { className: "px-4 pb-3", children: /* @__PURE__ */ jsx(
          InlineItemEditor,
          {
            item,
            columns,
...
function DefaultListItemContent({
  ...
  return /* @__PURE__ */ jsxs("div", { className: "flex flex-1 items-center gap-3 min-w-0", children: [
    descriptor.avatar && !!item[descriptor.avatar] && /* @__PURE__ */ jsx(
      "img",
      {
    ...
    !descriptor.avatar && IconComponent && /* @__PURE__ */ jsx("div", { className: "flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full bg-muted", children: /* @__PURE__ */ jsx(IconComponent, { className: "h-4 w-4 text-muted-foreground" }) }),
    !descriptor.avatar && iconField && !!item[iconField] && /* @__PURE__ */ jsx("div", { className: "flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full bg-muted", children: /* @__PURE__ */ jsx("span", { className: "text-sm", children: String(item[iconField]) }) }),
    /* @__PURE__ */ jsxs("div", { className: "flex-1 min-w-0", children: [
      /* @__PURE__ */ jsx("p", { className: "truncate text-sm font-medium", children: String(item[descriptor.title] ?? "") }),
      descriptor.subtitle && /* @__PURE__ */ jsx("p", { className: "truncate text-xs text-muted-foreground", children: String(item[descriptor.subtitle] ?? "") })
    ] }),
    descriptor.badges && descriptor.badges.length > 0 && /* @__PURE__ */ jsx("div", { className: "flex flex-shrink-0 gap-1", children: descriptor.badges.map((badge) => {
      const val = String(item[badge.field] ?? "");
      const opt = badge.options?.find((o) => o.value === val);
      return /* @__PURE__ */ jsx(
        "span",
    ...
    descriptor.metadata && descriptor.metadata.length > 0 && /* @__PURE__ */ jsx("div", { className: "hidden flex-shrink-0 gap-4 sm:flex", children: descriptor.metadata.map((meta) => {
      const val = item[meta.field];
      return /* @__PURE__ */ jsx("span", { className: "text-xs text-muted-foreground", children: meta.format ? meta.format(val) : String(val ?? "") }, meta.field);
    }) })
  ] });
}
function ViewModeSwitcher({
  ...
  return /* @__PURE__ */ jsx("div", { className: cn("inline-flex items-center rounded-lg bg-muted/60 p-1 gap-0.5", className), children: modes.filter((m) => enabledModes.includes(m.key)).map(({ key, icon: Icon, label }) => /* @__PURE__ */ jsxs(
    "button",
    {
      type: "button",
...
function TableIcon({ className }) {
  return /* @__PURE__ */ jsxs("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: 2, strokeLinecap: "round", strokeLinejoin: "round", className, "aria-hidden": true, children: [
    /* @__PURE__ */ jsx("title", { children: "Table" }),
...
function ListIcon({ className }) {
  return /* @__PURE__ */ jsxs("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: 2, strokeLinecap: "round", strokeLinejoin: "round", className, "aria-hidden": true, children: [
    /* @__PURE__ */ jsx("title", { children: "List" }),
...
function DataTableToolbar({
  ...
  return /* @__PURE__ */ jsxs("div", { className: cn("flex items-center justify-between gap-2", className), children: [
    /* @__PURE__ */ jsxs("div", { className: "flex flex-1 items-center gap-2", children: [
      ...
      hasFilters && /* @__PURE__ */ jsxs("div", { className: "flex items-center gap-1", children: [
        columnFilters.map((f) => /* @__PURE__ */ jsxs(
          "span",
          {
    ...
    /* @__PURE__ */ jsxs("div", { className: "flex items-center gap-1.5", children: [
      ...
      showColumnVisibility && viewMode === "table" && /* @__PURE__ */ jsxs("div", { className: "relative", children: [
        /* @__PURE__ */ jsxs(
          "button",
          {
            type: "button",
            ...
            children: [
              /* @__PURE__ */ jsx(ColumnsIcon, { className: "h-4 w-4" }),
              "Columns"
            ]
          }
        ),
        colVisOpen && /* @__PURE__ */ jsx(
      ...
      onRefresh && /* @__PURE__ */ jsx(
        "button",
        {
          ...
          children: /* @__PURE__ */ jsx(RefreshIcon, { className: "h-4 w-4" })
        }
      )
    ] })
  ] });
...
function ColumnVisibilityMenu({
  ...
  return /* @__PURE__ */ jsxs(Fragment, { children: [
    ...
    /* @__PURE__ */ jsxs("div", { className: "absolute right-0 z-50 mt-1 min-w-[180px] rounded-xl bg-popover p-2 shadow-xl", children: [
      /* @__PURE__ */ jsx("div", { className: "mb-2 px-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground", children: "Toggle columns" }),
...
function ColumnsIcon({ className }) {
  return /* @__PURE__ */ jsxs("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: 2, strokeLinecap: "round", strokeLinejoin: "round", className, "aria-hidden": true, children: [
    /* @__PURE__ */ jsx("title", { children: "Columns" }),
    /* @__PURE__ */ jsx("rect", { width: "18", height: "18", x: "3", y: "3", rx: "2", ry: "2" }),
...
function RefreshIcon({ className }) {
  return /* @__PURE__ */ jsxs("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: 2, strokeLinecap: "round", strokeLinejoin: "round", className, "aria-hidden": true, children: [
    /* @__PURE__ */ jsx("title", { children: "Refresh" }),
...
function DataTablePagination({
  ...
  return /* @__PURE__ */ jsxs(
    "div",
    {
      ...
      children: [
        /* @__PURE__ */ jsx("div", { className: "flex-1 text-sm text-muted-foreground", children: count > 0 && /* @__PURE__ */ jsxs("span", { children: [
          "Showing ",
        ...
        /* @__PURE__ */ jsxs("div", { className: "flex items-center gap-6 lg:gap-8", children: [
          /* @__PURE__ */ jsxs("div", { className: "flex items-center gap-2", children: [
            /* @__PURE__ */ jsx("label", { htmlFor: pageSizeId, className: "text-sm font-medium", children: "Rows per page" }),
            /* @__PURE__ */ jsx(
              "select",
              {
                id: pageSizeId,
          ...
          /* @__PURE__ */ jsxs("div", { className: "flex items-center gap-1 text-sm font-medium", children: [
            "Page ",
          ...
          /* @__PURE__ */ jsxs("div", { className: "flex items-center gap-1", children: [
            /* @__PURE__ */ jsx(
              PaginationButton,
              {
                ...
                label: "First page",
                children: /* @__PURE__ */ jsx(ChevronsLeftIcon, { className: "h-4 w-4" })
              }
            ),
            /* @__PURE__ */ jsx(
              PaginationButton,
            ...
            /* @__PURE__ */ jsx(
              PaginationButton,
              {
                ...
                disabled: !table.getCanNextPage(),
                label: "Last page",
                children: /* @__PURE__ */ jsx(ChevronsRightIcon, { className: "h-4 w-4" })
              }
            )
          ] })
        ] })
...
function ChevronLeftIcon({ className }) {
  return /* @__PURE__ */ jsxs("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: 2, strokeLinecap: "round", strokeLinejoin: "round", className, "aria-hidden": true, children: [
    /* @__PURE__ */ jsx("title", { children: "Chevron left" }),
...
function ChevronsLeftIcon({ className }) {
  return /* @__PURE__ */ jsxs("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: 2, strokeLinecap: "round", strokeLinejoin: "round", className, "aria-hidden": true, children: [
    /* @__PURE__ */ jsx("title", { children: "Chevrons left" }),
...
function ChevronsRightIcon({ className }) {
  return /* @__PURE__ */ jsxs("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: 2, strokeLinecap: "round", strokeLinejoin: "round", className, "aria-hidden": true, children: [
    /* @__PURE__ */ jsx("title", { children: "Chevrons right" }),
...
function EmptyState({ config, isFiltered = false, className }) {
  ...
  return /* @__PURE__ */ jsxs(
    "div",
    {
      ...
      children: [
        IconComponent && /* @__PURE__ */ jsx("div", { className: "mb-4 rounded-full bg-muted p-3", children: /* @__PURE__ */ jsx(IconComponent, { className: "h-8 w-8 text-muted-foreground" }) }),
        !IconComponent && /* @__PURE__ */ jsx("div", { className: "mb-4 rounded-full bg-muted p-3", children: /* @__PURE__ */ jsx(EmptyBoxIcon, { className: "h-8 w-8 text-muted-foreground" }) }),
        /* @__PURE__ */ jsx("h3", { className: "mb-1 text-base font-semibold", children: title }),
        /* @__PURE__ */ jsx("p", { className: "mb-4 max-w-sm text-sm text-muted-foreground", children: description }),
        action && /* @__PURE__ */ jsx(
          "button",
          {
            type: "button",
...
function EmptyBoxIcon({ className }) {
  return /* @__PURE__ */ jsxs("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: 1.5, strokeLinecap: "round", strokeLinejoin: "round", className, "aria-hidden": true, children: [
    /* @__PURE__ */ jsx("title", { children: "Empty" }),
    ...
    /* @__PURE__ */ jsx("line", { x1: "12", y1: "22", x2: "12", y2: "12" })
  ] });
}
    ...
    "div",
    {
      ...
      children: [
        /* @__PURE__ */ jsxs("div", { className: "flex items-center gap-3", children: [
          ...
          totalCount != null && totalCount > selectedCount && /* @__PURE__ */ jsxs(
            "button",
            {
              type: "button",
              onClick: () => {
        ...
        /* @__PURE__ */ jsx("div", { className: "flex items-center gap-2", children: batchActions.map((action) => /* @__PURE__ */ jsxs(
          "button",
          {
            type: "button",
            ...
            children: [
              action.icon && /* @__PURE__ */ jsx(action.icon, { className: "h-3.5 w-3.5" }),
              action.label
            ]
          },
          action.id
  ...
  return /* @__PURE__ */ jsxs("div", { className: cn("relative inline-block", className), children: [
    /* @__PURE__ */ jsxs(
      "button",
      {
        ...
        children: [
          /* @__PURE__ */ jsx(BookmarkIcon, { className: "h-4 w-4" }),
          "Presets",
          pendingChangesCount > 0 && /* @__PURE__ */ jsx("span", { className: "ml-1 inline-flex h-4 w-4 items-center justify-center rounded-full bg-orange-500 text-[10px] text-white", children: pendingChangesCount })
        ]
      }
    ),
    isOpen && /* @__PURE__ */ jsxs(
      "div",
      {
        ...
        children: [
          ...
          /* @__PURE__ */ jsx("div", { className: "max-h-[300px] overflow-auto p-2", children: activeTab === "filters" ? /* @__PURE__ */ jsxs(Fragment, { children: [
            activeFilterId && /* @__PURE__ */ jsx(
              "button",
              {
                type: "button",
                onClick: () => {
            ...
            filterPresets.map((preset) => /* @__PURE__ */ jsx(
              PresetItem,
              {
                name: preset.name,
            ...
            filterPresets.length === 0 && /* @__PURE__ */ jsx("p", { className: "px-3 py-4 text-center text-xs text-muted-foreground", children: "No saved filter presets" }),
            /* @__PURE__ */ jsxs(
              "button",
              {
                type: "button",
            ...
            columnPresets.map((preset) => /* @__PURE__ */ jsx(
              PresetItem,
              {
                name: preset.name,
                description: preset.description,
            ...
            columnPresets.length === 0 && /* @__PURE__ */ jsx("p", { className: "px-3 py-4 text-center text-xs text-muted-foreground", children: "No saved column presets" }),
            /* @__PURE__ */ jsxs(
              "button",
              {
                type: "button",
      ...
      children: [
        /* @__PURE__ */ jsxs(
          "button",
          {
            ...
            children: [
              /* @__PURE__ */ jsxs("span", { className: "flex items-center gap-1 text-sm font-medium truncate", children: [
                name,
                isDefault && /* @__PURE__ */ jsx("span", { className: "text-[10px] text-muted-foreground", children: "(default)" }),
                isActive && /* @__PURE__ */ jsx("span", { className: "h-1.5 w-1.5 rounded-full bg-primary" })
              ] }),
              description && /* @__PURE__ */ jsx("span", { className: "text-xs text-muted-foreground truncate", children: description })
            ]
          }
        ),
        /* @__PURE__ */ jsxs("div", { className: "hidden items-center gap-0.5 group-hover:flex", children: [
          /* @__PURE__ */ jsx(
            "button",
            {
              type: "button",
              ...
              children: /* @__PURE__ */ jsx(PencilIcon2, { className: "h-3 w-3" })
            }
          ),
          showDeleteConfirm ? /* @__PURE__ */ jsx(
            ...
            "button",
            {
              ...
              children: /* @__PURE__ */ jsx(TrashIcon2, { className: "h-3 w-3" })
            }
          )
        ] })
      ]
...
function BookmarkIcon({ className }) {
  return /* @__PURE__ */ jsxs("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: 2, strokeLinecap: "round", strokeLinejoin: "round", className, "aria-hidden": true, children: [
    /* @__PURE__ */ jsx("title", { children: "Presets" }),
    /* @__PURE__ */ jsx("path", { d: "m19 21-7-4-7 4V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2v16z" })
  ] });
}
...
function FilterPresetDialog({
  ...
  if (!open) return null;
  return /* @__PURE__ */ jsx("div", { className: "fixed inset-0 z-50 flex items-center justify-center bg-black/50", children: /* @__PURE__ */ jsxs("div", { className: "w-full max-w-lg rounded-lg border bg-background shadow-lg", children: [
    /* @__PURE__ */ jsxs("div", { className: "flex items-center justify-between border-b px-6 py-4", children: [
      /* @__PURE__ */ jsx("h2", { className: "text-lg font-semibold", children: preset ? "Edit Filter Preset" : "New Filter Preset" }),
      /* @__PURE__ */ jsx(
        "button",
        {
          type: "button",
          ...
          children: /* @__PURE__ */ jsx(XIcon2, { className: "h-4 w-4" })
        }
      )
    ] }),
    /* @__PURE__ */ jsxs("div", { className: "space-y-4 px-6 py-4", children: [
      /* @__PURE__ */ jsxs("div", { className: "space-y-2", children: [
        /* @__PURE__ */ jsx("label", { htmlFor: nameId, className: "text-sm font-medium", children: "Name" }),
        /* @__PURE__ */ jsx(
          "input",
          {
            id: nameId,
      ...
      /* @__PURE__ */ jsxs("div", { className: "space-y-2", children: [
        /* @__PURE__ */ jsx("label", { htmlFor: descriptionId, className: "text-sm font-medium", children: "Description" }),
        /* @__PURE__ */ jsx(
          "input",
          {
            id: descriptionId,
      ...
      /* @__PURE__ */ jsxs("fieldset", { className: "space-y-2 border-0 p-0", children: [
        /* @__PURE__ */ jsx("legend", { className: "sr-only", children: "Filter conditions" }),
        /* @__PURE__ */ jsxs("div", { className: "flex items-center justify-between", children: [
          /* @__PURE__ */ jsx("span", { className: "text-sm font-medium", children: "Conditions" }),
          /* @__PURE__ */ jsxs("div", { className: "flex items-center gap-1 rounded-md border p-0.5", children: [
            /* @__PURE__ */ jsx(
              "button",
              {
                type: "button",
        ...
        /* @__PURE__ */ jsx("div", { className: "space-y-2", children: clauses.map((clause, idx) => /* @__PURE__ */ jsxs("div", { className: "flex items-center gap-2", children: [
          idx > 0 && /* @__PURE__ */ jsx("span", { className: "text-xs text-muted-foreground w-8 text-center", children: logic.toUpperCase() }),
          idx === 0 && /* @__PURE__ */ jsx("span", { className: "w-8 text-center text-xs text-muted-foreground", children: "Where" }),
          /* @__PURE__ */ jsx(
            "select",
            {
              value: clause.field,
              ...
              children: filterableColumns.map((col) => /* @__PURE__ */ jsx("option", { value: col.accessorKey ?? col.id, children: typeof col.header === "string" ? col.header : col.accessorKey ?? col.id }, col.accessorKey ?? col.id))
            }
          ),
          /* @__PURE__ */ jsx(
            "select",
          ...
          /* @__PURE__ */ jsx(
            "button",
            {
              ...
              children: /* @__PURE__ */ jsx(XIcon2, { className: "h-3.5 w-3.5" })
            }
          )
        ] }, clause.id)) }),
        /* @__PURE__ */ jsxs(
          "button",
          {
            ...
            children: [
              /* @__PURE__ */ jsx(PlusIcon2, { className: "h-3 w-3" }),
              "Add condition"
            ]
          }
        )
    ...
    /* @__PURE__ */ jsxs("div", { className: "flex justify-end gap-2 border-t px-6 py-4", children: [
      /* @__PURE__ */ jsx(
        "button",
        {
          type: "button",
...
function ColumnPresetDialog({
  ...
  return /* @__PURE__ */ jsx("div", { className: "fixed inset-0 z-50 flex items-center justify-center bg-black/50", children: /* @__PURE__ */ jsxs("div", { className: "w-full max-w-lg rounded-lg border bg-background shadow-lg", children: [
    /* @__PURE__ */ jsxs("div", { className: "flex items-center justify-between border-b px-6 py-4", children: [
      /* @__PURE__ */ jsx("h2", { className: "text-lg font-semibold", children: preset ? "Edit Column Preset" : "New Column Preset" }),
      /* @__PURE__ */ jsx(
        "button",
        {
          type: "button",
          ...
          className: "rounded-sm opacity-70 hover:opacity-100",
          "aria-label": "Close dialog",
          children: /* @__PURE__ */ jsx(XIcon3, { className: "h-4 w-4" })
        }
      )
    ] }),
    /* @__PURE__ */ jsxs("div", { className: "space-y-4 px-6 py-4", children: [
      /* @__PURE__ */ jsxs("div", { className: "space-y-2", children: [
        /* @__PURE__ */ jsx("label", { htmlFor: "column-preset-name", className: "text-sm font-medium", children: "Name" }),
        /* @__PURE__ */ jsx(
          "input",
          {
      ...
      /* @__PURE__ */ jsxs("div", { className: "space-y-2", children: [
        /* @__PURE__ */ jsx("label", { htmlFor: "column-preset-description", className: "text-sm font-medium", children: "Description" }),
        /* @__PURE__ */ jsx(
          "input",
          {
      ...
      /* @__PURE__ */ jsxs("fieldset", { className: "space-y-2 border-0 p-0", children: [
        /* @__PURE__ */ jsx("legend", { className: "text-sm font-medium", children: "Columns" }),
        /* @__PURE__ */ jsx(
          "ul",
          {
            className: "max-h-[300px] list-none overflow-auto rounded-md border p-0",
            "aria-label": "Column order and visibility",
            children: entries.map((entry, idx) => /* @__PURE__ */ jsxs(
              "li",
              {
                ...
                children: [
                  ...
                  /* @__PURE__ */ jsx("span", { className: "flex-1 text-sm font-medium truncate", children: getColumnLabel(entry.id) }),
                  /* @__PURE__ */ jsxs("div", { className: "flex items-center gap-2", children: [
                    /* @__PURE__ */ jsx(
                      "input",
                      {
...
function EntityListView(props) {
  ...
  const [editingFilterPreset, setEditingFilterPreset] = useState(null);
  const [editingColumnPreset, setEditingColumnPreset] = useState(null);
  ...
  return /* @__PURE__ */ jsx(SelectionContext.Provider, { value: selectionStoreRef.current, children: /* @__PURE__ */ jsxs("div", { className: cn("flex flex-col gap-3", className), children: [
    /* @__PURE__ */ jsx(
      DataTableToolbar,
      {
        table,
        ...
        children: enablePresets && tableId && /* @__PURE__ */ jsx(
          PresetPicker,
          {
    ...
    isEmpty || isFilteredEmpty ? /* @__PURE__ */ jsx(
      EmptyState,
      {
        config: emptyState,
    ...
    enableMultiSelect && /* @__PURE__ */ jsx(
      MultiSelectBar,
      {
    ...
    enablePresets && /* @__PURE__ */ jsxs(Fragment, { children: [
      /* @__PURE__ */ jsx(
        FilterPresetDialog,
        {
  ...
  return /* @__PURE__ */ jsxs("div", { className: cn("relative inline-block", className), children: [
    /* @__PURE__ */ jsxs(
      "button",
      {
        ...
        children: [
          /* @__PURE__ */ jsx(FilterIcon, { className: "mr-1 h-3 w-3" }),
          "Filter"
        ]
      }
    ),
    isOpen && /* @__PURE__ */ jsx(
      "div",
      {
        ...
        children: /* @__PURE__ */ jsx(FilterControl, { column, filterType, onClose: () => setIsOpen(false) })
      }
    )
  ] });
}
function FilterControl({
  ...
  switch (filterType) {
    case "text":
      return /* @__PURE__ */ jsx(TextFilter, { column });
    case "number":
      return /* @__PURE__ */ jsx(NumberFilter, { column });
    case "boolean":
      return /* @__PURE__ */ jsx(BooleanFilter, { column });
    case "enum":
      return /* @__PURE__ */ jsx(EnumFilter, { column });
    case "date":
    case "dateRange":
      return /* @__PURE__ */ jsx(DateFilter, { column });
    default:
      return /* @__PURE__ */ jsx(TextFilter, { column });
  }
}
function TextFilter({ column }) {
  ...
  return /* @__PURE__ */ jsxs("div", { className: "space-y-2", children: [
    /* @__PURE__ */ jsx("label", { htmlFor: id, className: "text-xs font-medium", children: "Contains" }),
    /* @__PURE__ */ jsx(
      "input",
      {
        id,
...
function NumberFilter({ column }) {
  ...
  return /* @__PURE__ */ jsxs("div", { className: "space-y-2", children: [
    /* @__PURE__ */ jsx("span", { className: "text-xs font-medium", children: "Range" }),
    /* @__PURE__ */ jsxs("div", { className: "flex items-center gap-2", children: [
      /* @__PURE__ */ jsx(
        "input",
        {
...
function BooleanFilter({ column }) {
  const value = column.getFilterValue();
  return /* @__PURE__ */ jsxs("div", { className: "space-y-2", children: [
    /* @__PURE__ */ jsx("p", { className: "text-xs font-medium", children: "Value" }),
    /* @__PURE__ */ jsx("div", { className: "flex gap-2", children: [
      { label: "All", val: void 0 },
      { label: "True", val: true },
      { label: "False", val: false }
    ].map(({ label, val }) => /* @__PURE__ */ jsx(
      "button",
      {
        type: "button",
...
function EnumFilter({ column }) {
  ...
  return /* @__PURE__ */ jsxs("fieldset", { className: "space-y-2 border-0 p-0", children: [
    /* @__PURE__ */ jsx("legend", { className: "text-xs font-medium", children: "Select values" }),
    /* @__PURE__ */ jsx("div", { className: "max-h-[200px] space-y-1 overflow-auto", children: options.map((opt) => /* @__PURE__ */ jsxs(
      "label",
      {
...
function DateFilter({ column }) {
  ...
  const value = column.getFilterValue() ?? [void 0, void 0];
  return /* @__PURE__ */ jsxs("div", { className: "space-y-2", children: [
    /* @__PURE__ */ jsx("span", { className: "text-xs font-medium", children: "Date range" }),
...
function FilterIcon({ className }) {
  return /* @__PURE__ */ jsxs("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: 2, strokeLinecap: "round", strokeLinejoin: "round", className, "aria-hidden": true, children: [
    /* @__PURE__ */ jsx("title", { children: "Filter" }),
    /* @__PURE__ */ jsx("polygon", { points: "22 3 2 3 10 12.46 10 19 14 21 14 12.46 22 3" })
  ] });
}
