...
- [ ] T1. NEW `src/shared/db/lookups-selectors.ts`:
...
- [ ] T4. NEW or MODIFY `src/app/__tests__/tier1-provider.spec.tsx`. Render `<Tier1Provider>` with a graph fixture; assert each lookup array's length + ordering; then upsert a new `entity_metadata` row in the store and assert the consumer re-renders with the new entry.
- [ ] T5. Hook surface check: ensure `useTier1()` consumers in the existing codebase still type-check (no API breakage — pure additions).
