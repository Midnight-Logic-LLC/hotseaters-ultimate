...
--
-- INSTEAD OF triggers on the view route writes to <entity>_local and append a
-- row to `local_writes`. A pg_notify on `local_write` drives the write-path
-- sync utility, which POSTs pending writes to Supabase and on confirmation
-- clears the optimistic row.
--
-- Electric does read-path sync only; the write path is the app's job — this
-- schema is that job, kept out of the store/UI code.
--
-- Self-hosted Supabase only. HotSeatersMVP is the bible.
-- Generated from migrations through 20260523000018.
-- =============================================================================

-- ---------------------------------------------------------------------------
-- _pglite_schema_version — single-row version stamp. Compared at boot in
-- pglite-client.ts (Change 4); if the bundled version is newer the worker
-- drops *_local + *_synced for migrated entities and re-hydrates.
-- ---------------------------------------------------------------------------
...
INSERT INTO _pglite_schema_version (id, version)
  VALUES (1, '20260523000018')
  ON CONFLICT (id) DO UPDATE SET version = EXCLUDED.version;

-- ---------------------------------------------------------------------------
-- _sync_meta — single-row table gating the load-once-on-login bootstrap.
-- `hydrated_at` is NULL until the first full shape hydration completes;
-- subsequent logins see it set and resume shapes incrementally from
-- Electric's persisted offsets. `tenant_id` records which tenant the local DB
-- currently holds, so a tenant switch can be detected and the cache cleared.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- local_writes — write-ahead log of local (optimistic) mutations awaiting
-- server confirmation. The write-sync drain (Change 4) reads pending rows
-- and POSTs them to Supabase REST.
-- ---------------------------------------------------------------------------
...
-- =============================================================================
-- TIER-A ENTITIES
-- Each block: <entity>_synced, <entity>_local, view, three trigger functions,
-- three INSTEAD-OF triggers.
-- =============================================================================

-- ── company ────────────────────────────────────────────────────────────────
-- Tenant root. Scoped locally by id = current_company_id.
...
CREATE OR REPLACE VIEW company AS
  ...
  UNION ALL
  SELECT l.id, l.legacy_id, l.name, l.email, l.sender_email, l.phone,
         l.website, l.address, l.city, l.state, l.zip, l.owner_id,
         l.subscription_tier, l.is_active, l.stripe_customer_id,
         l.marketplace_post_jobs, l.marketplace_fill_jobs, l.has_hsh_addon,
         l.hsh_rating, l.hsh_reviews_count, l.show_debug_info,
...
BEGIN
  INSERT INTO company_local (id, legacy_id, name, email, sender_email, phone,
    website, address, city, state, zip, owner_id, subscription_tier, is_active,
    stripe_customer_id, marketplace_post_jobs, marketplace_fill_jobs,
    has_hsh_addon, hsh_rating, hsh_reviews_count, show_debug_info,
    created_at, updated_at, is_sample, created_by_id, created_by)
  VALUES (COALESCE(NEW.id, gen_random_uuid()::text), NEW.legacy_id, NEW.name,
    NEW.email, NEW.sender_email, NEW.phone, NEW.website, NEW.address, NEW.city,
    NEW.state, NEW.zip, NEW.owner_id, NEW.subscription_tier, NEW.is_active,
    NEW.stripe_customer_id, NEW.marketplace_post_jobs, NEW.marketplace_fill_jobs,
    NEW.has_hsh_addon, NEW.hsh_rating, NEW.hsh_reviews_count, NEW.show_debug_info,
...
BEGIN
  INSERT INTO company_local (id, legacy_id, name, email, sender_email, phone,
    ...
    has_hsh_addon, hsh_rating, hsh_reviews_count, show_debug_info,
    created_at, updated_at, is_sample, created_by_id, created_by)
  VALUES (NEW.id, NEW.legacy_id, NEW.name, NEW.email, NEW.sender_email, NEW.phone,
    NEW.website, NEW.address, NEW.city, NEW.state, NEW.zip, NEW.owner_id,
    NEW.subscription_tier, NEW.is_active, NEW.stripe_customer_id,
    NEW.marketplace_post_jobs, NEW.marketplace_fill_jobs, NEW.has_hsh_addon,
    NEW.hsh_rating, NEW.hsh_reviews_count, NEW.show_debug_info,
    NEW.created_at, NEW.updated_at, NEW.is_sample, NEW.created_by_id, NEW.created_by)
  ON CONFLICT (id) DO UPDATE SET
    legacy_id = EXCLUDED.legacy_id, name = EXCLUDED.name, email = EXCLUDED.email,
    sender_email = EXCLUDED.sender_email, phone = EXCLUDED.phone,
    website = EXCLUDED.website, address = EXCLUDED.address, city = EXCLUDED.city,
    state = EXCLUDED.state, zip = EXCLUDED.zip, owner_id = EXCLUDED.owner_id,
    subscription_tier = EXCLUDED.subscription_tier, is_active = EXCLUDED.is_active,
    stripe_customer_id = EXCLUDED.stripe_customer_id,
    marketplace_post_jobs = EXCLUDED.marketplace_post_jobs,
    marketplace_fill_jobs = EXCLUDED.marketplace_fill_jobs,
    has_hsh_addon = EXCLUDED.has_hsh_addon, hsh_rating = EXCLUDED.hsh_rating,
    hsh_reviews_count = EXCLUDED.hsh_reviews_count,
    show_debug_info = EXCLUDED.show_debug_info,
    updated_at = EXCLUDED.updated_at, is_sample = EXCLUDED.is_sample,
    created_by_id = EXCLUDED.created_by_id, created_by = EXCLUDED.created_by,
    is_deleted = false;
  INSERT INTO local_writes (entity, operation, row_id, payload)
...
CREATE OR REPLACE VIEW user_info AS
  ...
  UNION ALL
  SELECT l.id, l.legacy_id, l.first_name, l.last_name, l.title, l.phone,
         l.profile_photo, l.signature_image, l.notes, l.company_id,
         l.company_role, l.user_id, l.status, l.consultant_tier_id,
         l.is_sales, l.hsh_location, l.hsh_rating, l.hsh_reviews_count,
         l.hsh_skills, l.hsh_profile_description, l.created_at, l.updated_at,
         l.is_sample, l.created_by_id, l.created_by
  FROM user_info_local l
  WHERE NOT l.is_deleted;

CREATE OR REPLACE FUNCTION user_info_insert() RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO user_info_local (id, legacy_id, first_name, last_name, title,
    phone, profile_photo, signature_image, notes, company_id, company_role,
    user_id, status, consultant_tier_id, is_sales, hsh_location, hsh_rating,
    hsh_reviews_count, hsh_skills, hsh_profile_description, created_at,
    updated_at, is_sample, created_by_id, created_by)
  VALUES (COALESCE(NEW.id, gen_random_uuid()::text), NEW.legacy_id,
    NEW.first_name, NEW.last_name, NEW.title, NEW.phone, NEW.profile_photo,
    NEW.signature_image, NEW.notes, NEW.company_id, NEW.company_role,
    NEW.user_id, NEW.status, NEW.consultant_tier_id, NEW.is_sales,
    NEW.hsh_location, NEW.hsh_rating, NEW.hsh_reviews_count, NEW.hsh_skills,
    NEW.hsh_profile_description, NEW.created_at, NEW.updated_at, NEW.is_sample,
...
BEGIN
  INSERT INTO user_info_local (id, legacy_id, first_name, last_name, title,
    ...
    hsh_reviews_count, hsh_skills, hsh_profile_description, created_at,
    updated_at, is_sample, created_by_id, created_by)
  VALUES (NEW.id, NEW.legacy_id, NEW.first_name, NEW.last_name, NEW.title,
    NEW.phone, NEW.profile_photo, NEW.signature_image, NEW.notes, NEW.company_id,
    NEW.company_role, NEW.user_id, NEW.status, NEW.consultant_tier_id,
    NEW.is_sales, NEW.hsh_location, NEW.hsh_rating, NEW.hsh_reviews_count,
    NEW.hsh_skills, NEW.hsh_profile_description, NEW.created_at, NEW.updated_at,
    NEW.is_sample, NEW.created_by_id, NEW.created_by)
  ON CONFLICT (id) DO UPDATE SET
    legacy_id = EXCLUDED.legacy_id, first_name = EXCLUDED.first_name,
    last_name = EXCLUDED.last_name, title = EXCLUDED.title, phone = EXCLUDED.phone,
    profile_photo = EXCLUDED.profile_photo, signature_image = EXCLUDED.signature_image,
    notes = EXCLUDED.notes, company_id = EXCLUDED.company_id,
    company_role = EXCLUDED.company_role, user_id = EXCLUDED.user_id,
    status = EXCLUDED.status, consultant_tier_id = EXCLUDED.consultant_tier_id,
    is_sales = EXCLUDED.is_sales, hsh_location = EXCLUDED.hsh_location,
    hsh_rating = EXCLUDED.hsh_rating, hsh_reviews_count = EXCLUDED.hsh_reviews_count,
...
-- ── metadata_type ──────────────────────────────────────────────────────────
-- NOTE: server column "order" is a reserved word; quoted everywhere.
...
CREATE OR REPLACE VIEW metadata_type AS
  ...
  UNION ALL
  SELECT l.id, l.company_id, l.scope, l.name, l.description, l.is_active,
         l.is_default, l."order", l.extra_schema, l.created_at, l.updated_at,
         l.is_sample, l.created_by_id, l.created_by
...
BEGIN
  INSERT INTO metadata_type_local (id, company_id, scope, name, description,
    is_active, is_default, "order", extra_schema, created_at, updated_at,
    is_sample, created_by_id, created_by)
  VALUES (COALESCE(NEW.id, gen_random_uuid()::text), NEW.company_id, NEW.scope,
    NEW.name, NEW.description, NEW.is_active, NEW.is_default, NEW."order",
    NEW.extra_schema, NEW.created_at, NEW.updated_at, NEW.is_sample,
    NEW.created_by_id, NEW.created_by);
  INSERT INTO local_writes (entity, operation, row_id, payload)
...
BEGIN
  INSERT INTO metadata_type_local (id, company_id, scope, name, description,
    is_active, is_default, "order", extra_schema, created_at, updated_at,
    is_sample, created_by_id, created_by)
  VALUES (NEW.id, NEW.company_id, NEW.scope, NEW.name, NEW.description,
    NEW.is_active, NEW.is_default, NEW."order", NEW.extra_schema,
    NEW.created_at, NEW.updated_at, NEW.is_sample, NEW.created_by_id, NEW.created_by)
  ON CONFLICT (id) DO UPDATE SET
    company_id = EXCLUDED.company_id, scope = EXCLUDED.scope,
    name = EXCLUDED.name, description = EXCLUDED.description,
    is_active = EXCLUDED.is_active, is_default = EXCLUDED.is_default,
    "order" = EXCLUDED."order", extra_schema = EXCLUDED.extra_schema,
    updated_at = EXCLUDED.updated_at, is_sample = EXCLUDED.is_sample,
    created_by_id = EXCLUDED.created_by_id, created_by = EXCLUDED.created_by,
    is_deleted = false;
  INSERT INTO local_writes (entity, operation, row_id, payload)
...
CREATE OR REPLACE VIEW entity_metadata AS
  ...
  UNION ALL
  SELECT l.id, l.metadata_type_id, l.company_id, l.multiplier, l.extra,
         l.created_at, l.updated_at, l.is_sample, l.created_by_id, l.created_by
...
BEGIN
  INSERT INTO entity_metadata_local (id, metadata_type_id, company_id, multiplier,
    extra, created_at, updated_at, is_sample, created_by_id, created_by)
  VALUES (COALESCE(NEW.id, gen_random_uuid()::text), NEW.metadata_type_id,
    NEW.company_id, NEW.multiplier, NEW.extra, NEW.created_at, NEW.updated_at,
    NEW.is_sample, NEW.created_by_id, NEW.created_by);
  INSERT INTO local_writes (entity, operation, row_id, payload)
...
BEGIN
  INSERT INTO entity_metadata_local (id, metadata_type_id, company_id, multiplier,
    extra, created_at, updated_at, is_sample, created_by_id, created_by)
  VALUES (NEW.id, NEW.metadata_type_id, NEW.company_id, NEW.multiplier,
    NEW.extra, NEW.created_at, NEW.updated_at, NEW.is_sample, NEW.created_by_id,
    NEW.created_by)
  ON CONFLICT (id) DO UPDATE SET
    metadata_type_id = EXCLUDED.metadata_type_id,
    company_id = EXCLUDED.company_id, multiplier = EXCLUDED.multiplier,
    extra = EXCLUDED.extra, updated_at = EXCLUDED.updated_at,
    is_sample = EXCLUDED.is_sample, created_by_id = EXCLUDED.created_by_id,
    created_by = EXCLUDED.created_by, is_deleted = false;
  INSERT INTO local_writes (entity, operation, row_id, payload)
...
CREATE OR REPLACE VIEW entity_setting AS
  ...
  UNION ALL
  SELECT l.id, l.settings_type_id, l.company_id, l.user_info_id,
         l.document_template_id, l.data, l.created_at, l.updated_at,
         l.is_sample, l.created_by_id, l.created_by
...
BEGIN
  INSERT INTO entity_setting_local (id, settings_type_id, company_id,
    user_info_id, document_template_id, data, created_at, updated_at,
    is_sample, created_by_id, created_by)
  VALUES (COALESCE(NEW.id, gen_random_uuid()::text), NEW.settings_type_id,
    NEW.company_id, NEW.user_info_id, NEW.document_template_id, NEW.data,
    NEW.created_at, NEW.updated_at, NEW.is_sample, NEW.created_by_id, NEW.created_by);
  INSERT INTO local_writes (entity, operation, row_id, payload)
...
BEGIN
  INSERT INTO entity_setting_local (id, settings_type_id, company_id,
    user_info_id, document_template_id, data, created_at, updated_at,
    is_sample, created_by_id, created_by)
  VALUES (NEW.id, NEW.settings_type_id, NEW.company_id, NEW.user_info_id,
    NEW.document_template_id, NEW.data, NEW.created_at, NEW.updated_at,
    NEW.is_sample, NEW.created_by_id, NEW.created_by)
  ON CONFLICT (id) DO UPDATE SET
    settings_type_id = EXCLUDED.settings_type_id,
    company_id = EXCLUDED.company_id, user_info_id = EXCLUDED.user_info_id,
...
CREATE OR REPLACE VIEW client AS
  ...
  UNION ALL
  SELECT l.id, l.legacy_id, l.company_id, l.client_type_id, l.firm_name,
         l.phone, l.website, l.sales_lead, l.is_lead, l.status, l.notes,
         l.created_at, l.updated_at, l.is_sample, l.created_by_id, l.created_by
  FROM client_local l
  WHERE NOT l.is_deleted;
...
BEGIN
  INSERT INTO client_local (id, legacy_id, company_id, client_type_id, firm_name,
    phone, website, sales_lead, is_lead, status, notes, created_at, updated_at,
    is_sample, created_by_id, created_by)
  VALUES (COALESCE(NEW.id, gen_random_uuid()::text), NEW.legacy_id, NEW.company_id,
    NEW.client_type_id, NEW.firm_name, NEW.phone, NEW.website, NEW.sales_lead,
    NEW.is_lead, NEW.status, NEW.notes, NEW.created_at, NEW.updated_at,
    NEW.is_sample, NEW.created_by_id, NEW.created_by);
  INSERT INTO local_writes (entity, operation, row_id, payload)
...
BEGIN
  INSERT INTO client_local (id, legacy_id, company_id, client_type_id, firm_name,
    phone, website, sales_lead, is_lead, status, notes, created_at, updated_at,
    is_sample, created_by_id, created_by)
  VALUES (NEW.id, NEW.legacy_id, NEW.company_id, NEW.client_type_id, NEW.firm_name,
    NEW.phone, NEW.website, NEW.sales_lead, NEW.is_lead, NEW.status, NEW.notes,
    NEW.created_at, NEW.updated_at, NEW.is_sample, NEW.created_by_id, NEW.created_by)
  ON CONFLICT (id) DO UPDATE SET
    legacy_id = EXCLUDED.legacy_id, company_id = EXCLUDED.company_id,
    client_type_id = EXCLUDED.client_type_id, firm_name = EXCLUDED.firm_name,
    phone = EXCLUDED.phone, website = EXCLUDED.website,
    sales_lead = EXCLUDED.sales_lead, is_lead = EXCLUDED.is_lead,
...
CREATE OR REPLACE VIEW client_address AS
  ...
  UNION ALL
  SELECT l.id, l.legacy_id, l.company_id, l.client_id, l.label, l.address,
         l.city, l.state, l.zip, l.is_primary, l.created_at, l.updated_at,
         l.is_sample, l.created_by_id, l.created_by
...
BEGIN
  INSERT INTO client_address_local (id, legacy_id, company_id, client_id, label,
    address, city, state, zip, is_primary, created_at, updated_at, is_sample,
    created_by_id, created_by)
  VALUES (COALESCE(NEW.id, gen_random_uuid()::text), NEW.legacy_id, NEW.company_id,
    NEW.client_id, NEW.label, NEW.address, NEW.city, NEW.state, NEW.zip,
    NEW.is_primary, NEW.created_at, NEW.updated_at, NEW.is_sample,
    NEW.created_by_id, NEW.created_by);
  INSERT INTO local_writes (entity, operation, row_id, payload)
...
BEGIN
  INSERT INTO client_address_local (id, legacy_id, company_id, client_id, label,
    address, city, state, zip, is_primary, created_at, updated_at, is_sample,
    created_by_id, created_by)
  VALUES (NEW.id, NEW.legacy_id, NEW.company_id, NEW.client_id, NEW.label,
    NEW.address, NEW.city, NEW.state, NEW.zip, NEW.is_primary,
    NEW.created_at, NEW.updated_at, NEW.is_sample, NEW.created_by_id, NEW.created_by)
  ON CONFLICT (id) DO UPDATE SET
    legacy_id = EXCLUDED.legacy_id, company_id = EXCLUDED.company_id,
    client_id = EXCLUDED.client_id, label = EXCLUDED.label,
    address = EXCLUDED.address, city = EXCLUDED.city, state = EXCLUDED.state,
    zip = EXCLUDED.zip, is_primary = EXCLUDED.is_primary,
    updated_at = EXCLUDED.updated_at, is_sample = EXCLUDED.is_sample,
    created_by_id = EXCLUDED.created_by_id, created_by = EXCLUDED.created_by,
    is_deleted = false;
  INSERT INTO local_writes (entity, operation, row_id, payload)
...
CREATE OR REPLACE VIEW trial AS
  ...
  UNION ALL
  SELECT l.id, l.legacy_id, l.company_id, l.client_id, l.pipeline_stage_id,
         l.deal_document_id, l.primary_contact_id, l.frp_client_id,
         l.frp_contact_id, l.consultant_id, l.case_name, l.case_number,
         l.case_type, l.case_style, l.job_number, l.side, l.judge, l.courthouse,
         l.court, l.city, l.state, l.notes, l.start_date, l.end_date,
         l.completion_date, l.continued_date_precision, l.estimated_value,
         l.retainer_value, l.daily_minimum_hours, l.bill_for_weekends,
         l.won_date, l.lost_date, l.completion_type,
         l.invoice_recipient_preferences, l.updated_by, l.created_at,
         l.updated_at, l.is_sample, l.created_by_id, l.created_by
  FROM trial_local l
  WHERE NOT l.is_deleted;

CREATE OR REPLACE FUNCTION trial_insert() RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO trial_local (id, legacy_id, company_id, client_id, pipeline_stage_id,
    deal_document_id, primary_contact_id, frp_client_id, frp_contact_id,
    consultant_id, case_name, case_number, case_type, case_style, job_number,
    side, judge, courthouse, court, city, state, notes, start_date, end_date,
    completion_date, continued_date_precision, estimated_value, retainer_value,
    daily_minimum_hours, bill_for_weekends, won_date, lost_date, completion_type,
    invoice_recipient_preferences, updated_by, created_at, updated_at, is_sample,
    created_by_id, created_by)
  VALUES (COALESCE(NEW.id, gen_random_uuid()::text), NEW.legacy_id, NEW.company_id,
    NEW.client_id, NEW.pipeline_stage_id, NEW.deal_document_id,
    NEW.primary_contact_id, NEW.frp_client_id, NEW.frp_contact_id,
    NEW.consultant_id, NEW.case_name, NEW.case_number, NEW.case_type,
    NEW.case_style, NEW.job_number, NEW.side, NEW.judge, NEW.courthouse,
    NEW.court, NEW.city, NEW.state, NEW.notes, NEW.start_date, NEW.end_date,
    NEW.completion_date, NEW.continued_date_precision, NEW.estimated_value,
    NEW.retainer_value, NEW.daily_minimum_hours, NEW.bill_for_weekends,
    NEW.won_date, NEW.lost_date, NEW.completion_type,
    NEW.invoice_recipient_preferences, NEW.updated_by, NEW.created_at,
    NEW.updated_at, NEW.is_sample, NEW.created_by_id, NEW.created_by);
  INSERT INTO local_writes (entity, operation, row_id, payload)
...
BEGIN
  INSERT INTO trial_local (id, legacy_id, company_id, client_id, pipeline_stage_id,
    ...
    invoice_recipient_preferences, updated_by, created_at, updated_at, is_sample,
    created_by_id, created_by)
  VALUES (NEW.id, NEW.legacy_id, NEW.company_id, NEW.client_id, NEW.pipeline_stage_id,
    NEW.deal_document_id, NEW.primary_contact_id, NEW.frp_client_id,
    NEW.frp_contact_id, NEW.consultant_id, NEW.case_name, NEW.case_number,
    NEW.case_type, NEW.case_style, NEW.job_number, NEW.side, NEW.judge,
    NEW.courthouse, NEW.court, NEW.city, NEW.state, NEW.notes, NEW.start_date,
    NEW.end_date, NEW.completion_date, NEW.continued_date_precision,
    NEW.estimated_value, NEW.retainer_value, NEW.daily_minimum_hours,
    NEW.bill_for_weekends, NEW.won_date, NEW.lost_date, NEW.completion_type,
    NEW.invoice_recipient_preferences, NEW.updated_by, NEW.created_at,
    NEW.updated_at, NEW.is_sample, NEW.created_by_id, NEW.created_by)
  ON CONFLICT (id) DO UPDATE SET
    legacy_id = EXCLUDED.legacy_id, company_id = EXCLUDED.company_id,
    client_id = EXCLUDED.client_id, pipeline_stage_id = EXCLUDED.pipeline_stage_id,
    ...
    consultant_id = EXCLUDED.consultant_id, case_name = EXCLUDED.case_name,
    case_number = EXCLUDED.case_number, case_type = EXCLUDED.case_type,
    case_style = EXCLUDED.case_style, job_number = EXCLUDED.job_number,
    side = EXCLUDED.side, judge = EXCLUDED.judge,
    courthouse = EXCLUDED.courthouse, court = EXCLUDED.court,
    city = EXCLUDED.city, state = EXCLUDED.state, notes = EXCLUDED.notes,
    ...
    won_date = EXCLUDED.won_date, lost_date = EXCLUDED.lost_date,
...
CREATE OR REPLACE VIEW trial_service AS
  ...
  UNION ALL
  SELECT l.id, l.legacy_id, l.company_id, l.trial_id, l.service_id, l.segment_id,
         l.service_name, l.service_phase, l.custom_description, l.rate_type,
         l.rate, l.travel_eligible, l.display_order, l.final_billing_method,
         l.split_billing_at_threshold, l.start_date, l.end_date,
         l.days_before_trial, l.estimated_quantity, l.estimated_total,
         l.estimated_travel_hours, l.pre_trial_estimated_hours,
         l.projected_daily_revenue, l.in_trial_projected_daily_revenue,
         l.pre_trial_projected_daily_revenue, l.updated_by, l.created_at,
         l.updated_at, l.is_sample, l.created_by_id, l.created_by
...
BEGIN
  INSERT INTO trial_service_local (id, legacy_id, company_id, trial_id, service_id,
    segment_id, service_name, service_phase, custom_description, rate_type, rate,
    travel_eligible, display_order, final_billing_method, split_billing_at_threshold,
    start_date, end_date, days_before_trial, estimated_quantity, estimated_total,
    estimated_travel_hours, pre_trial_estimated_hours, projected_daily_revenue,
    in_trial_projected_daily_revenue, pre_trial_projected_daily_revenue, updated_by,
    created_at, updated_at, is_sample, created_by_id, created_by)
  VALUES (COALESCE(NEW.id, gen_random_uuid()::text), NEW.legacy_id, NEW.company_id,
    NEW.trial_id, NEW.service_id, NEW.segment_id, NEW.service_name,
    NEW.service_phase, NEW.custom_description, NEW.rate_type, NEW.rate,
    NEW.travel_eligible, NEW.display_order, NEW.final_billing_method,
    NEW.split_billing_at_threshold, NEW.start_date, NEW.end_date,
    NEW.days_before_trial, NEW.estimated_quantity, NEW.estimated_total,
    NEW.estimated_travel_hours, NEW.pre_trial_estimated_hours,
    NEW.projected_daily_revenue, NEW.in_trial_projected_daily_revenue,
    NEW.pre_trial_projected_daily_revenue, NEW.updated_by, NEW.created_at,
    NEW.updated_at, NEW.is_sample, NEW.created_by_id, NEW.created_by);
  INSERT INTO local_writes (entity, operation, row_id, payload)
...
BEGIN
  INSERT INTO trial_service_local (id, legacy_id, company_id, trial_id, service_id,
    ...
    in_trial_projected_daily_revenue, pre_trial_projected_daily_revenue, updated_by,
    created_at, updated_at, is_sample, created_by_id, created_by)
  VALUES (NEW.id, NEW.legacy_id, NEW.company_id, NEW.trial_id, NEW.service_id,
    NEW.segment_id, NEW.service_name, NEW.service_phase, NEW.custom_description,
    NEW.rate_type, NEW.rate, NEW.travel_eligible, NEW.display_order,
    NEW.final_billing_method, NEW.split_billing_at_threshold, NEW.start_date,
    NEW.end_date, NEW.days_before_trial, NEW.estimated_quantity,
    NEW.estimated_total, NEW.estimated_travel_hours, NEW.pre_trial_estimated_hours,
    NEW.projected_daily_revenue, NEW.in_trial_projected_daily_revenue,
    NEW.pre_trial_projected_daily_revenue, NEW.updated_by, NEW.created_at,
    NEW.updated_at, NEW.is_sample, NEW.created_by_id, NEW.created_by)
  ON CONFLICT (id) DO UPDATE SET
    legacy_id = EXCLUDED.legacy_id, company_id = EXCLUDED.company_id,
    trial_id = EXCLUDED.trial_id, service_id = EXCLUDED.service_id,
    segment_id = EXCLUDED.segment_id, service_name = EXCLUDED.service_name,
...
CREATE OR REPLACE VIEW trial_contact AS
  ...
  UNION ALL
  SELECT l.id, l.legacy_id, l.company_id, l.trial_id, l.attorney_id,
         l.created_at, l.updated_at, l.is_sample, l.created_by_id, l.created_by
...
BEGIN
  INSERT INTO trial_contact_local (id, legacy_id, company_id, trial_id,
    attorney_id, created_at, updated_at, is_sample, created_by_id, created_by)
  VALUES (COALESCE(NEW.id, gen_random_uuid()::text), NEW.legacy_id, NEW.company_id,
    NEW.trial_id, NEW.attorney_id, NEW.created_at, NEW.updated_at, NEW.is_sample,
    NEW.created_by_id, NEW.created_by);
  INSERT INTO local_writes (entity, operation, row_id, payload)
...
BEGIN
  INSERT INTO trial_contact_local (id, legacy_id, company_id, trial_id,
    attorney_id, created_at, updated_at, is_sample, created_by_id, created_by)
  VALUES (NEW.id, NEW.legacy_id, NEW.company_id, NEW.trial_id, NEW.attorney_id,
    NEW.created_at, NEW.updated_at, NEW.is_sample, NEW.created_by_id, NEW.created_by)
  ON CONFLICT (id) DO UPDATE SET
    legacy_id = EXCLUDED.legacy_id, company_id = EXCLUDED.company_id,
    trial_id = EXCLUDED.trial_id, attorney_id = EXCLUDED.attorney_id,
    updated_at = EXCLUDED.updated_at, is_sample = EXCLUDED.is_sample,
    created_by_id = EXCLUDED.created_by_id, created_by = EXCLUDED.created_by,
    is_deleted = false;
  INSERT INTO local_writes (entity, operation, row_id, payload)
