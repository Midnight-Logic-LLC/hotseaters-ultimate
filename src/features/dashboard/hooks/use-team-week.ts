 ...
 *   • useEntityView<TimeEntryRow> (hybrid REST) — time entries in window
 *   • useEntityView<SubcontractAssignmentRow>   — HSH assignments
...
export function useTeamWeek(opts: UseTeamWeekOptions = {}): TeamWeekResult {
  ...
  const timeEntryView = useEntityView<TimeEntryRow>({
    type: 'TimeEntry',
    baseQueryKey: ['TimeEntry', 'window', companyId ?? '__none__', start.toISOString(), end.toISOString()],
    view: {},
    mode: 'hybrid',
    enabled: !!companyId,
    remoteFetch: async () => {
